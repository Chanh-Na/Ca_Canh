...
public class AquariumAdapter extends RecyclerView.Adapter<AquariumAdapter.MyViewHolder>{

    ArrayList<Aquarium> aquariums;
    Context context;

    public AquariumAdapter(ArrayList<Aquarium> aquariums, Context context) {
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        Aquarium aquarium = aquariums.get(position);
        holder.name.setText(aquarium.getName());

        DecimalFormat decimalFormat = new DecimalFormat("###,###,###");
        holder.price.setText(decimalFormat.format(aquarium.getPrice()) + " VND");
        Picasso.with(context).load(aquarium.getUrlImage())
                .fit()
        ...
        public MyViewHolder(View itemView) {
            ...
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View view) {
                    ShowDialog(aquariums.get(getPosition()));
                }
            });
        }
    }

    private void ShowDialog(final Aquarium aquarium){
        final Dialog dialog = new Dialog(context);
        dialog.setContentView(R.layout.aquarium_detail);
        ...
        TextView color = dialog.findViewById(R.id.color);

        name.setText(aquarium.getName());
        DecimalFormat decimalFormat = new DecimalFormat("###,###,###");
        price.setText(decimalFormat.format(aquarium.getPrice()) + " VND");
        Picasso.with(context).load(aquarium.getUrlImage())
                .fit()
                .placeholder(R.drawable.noimage)
                .error(R.drawable.errorimage)
        ...
        origin.setText("Nguồn gốc: " + aquarium.getOrigin());
        size.setText("Kích thước: " + aquarium.getSize() + "mm");
        brand.setText("Hãng sản xuất: " + aquarium.getBrand());
        material.setText("Chất liệu: " + aquarium.getMaterial());
        color.setText("Màu sắc: " + aquarium.getColor());
            ...
            public void onClick(View view) {
                ...
                if (MainActivity.carts.size() >0 ){
                    ...
                    for (int i = 0; i < MainActivity.carts.size(); i++){
                        if (MainActivity.carts.get(i).getName().equals( aquarium.getName())){
                            ...
                            MainActivity.carts.get(i).setPrice(aquarium.getPrice() * MainActivity.carts.get(i).getQuantity());
                            exists = true;
                        }
                    }

                    if (!exists){
                        long newPrice = quantity * aquarium.getPrice();
                        MainActivity.carts.add(new Cart(null, aquarium.getName(), newPrice, aquarium.getUrlImage(), quantity));
                    }
